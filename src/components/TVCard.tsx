...
import { Play, Plus, Star, Info, Check, Tv, Heart } from 'lucide-react';
...
interface TVCardProps {
  ...
  onToggleLike: (show: TVShow) => void;
  isLiked: boolean;
  index?: number;
}

const TVCard: React.FC<TVCardProps> = ({ show, onPlay, onInfo, onToggleLike, isLiked, index = 0 }) => {
  ...
  const poster = imgError ? '/no-movie.png' : getPosterUrl(show.poster_path, 'w500');
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-xl bg-[#1a242f] aspect-[2/3]">
        <img src={poster} alt={show.name} loading="lazy" onError={() => setImgError(true)}
        ...
        <div className="absolute top-2 left-2 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1 bg-[#00a8e1]/80 backdrop-blur-sm px-1.5 py-1 rounded-lg">
        ...
        <div className="absolute top-2 right-2 flex flex-col gap-1.5">
          <button
            onClick={(e) => { e.stopPropagation(); onToggleLike(show); }}
            title={isLiked ? 'Remove from liked' : 'Like this show'}
        ...
        <div className={`absolute bottom-0 left-0 right-0 p-3 flex flex-col gap-2 transition-all duration-300 ${hovered ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
          <button onClick={(e) => { e.stopPropagation(); onPlay(show); }}
          ...
          <div className="flex gap-2">
            <button onClick={(e) => { e.stopPropagation(); onInfo(show); }}
      ...
      <div className="mt-2.5 px-0.5">
        <h3 className="text-white font-semibold text-sm leading-tight line-clamp-1">{show.name}</h3>
        <div className="flex items-center gap-2 mt-1">
          <span className="text-[#8197a4] text-xs">{year}</span>
          <span className="text-[#8197a4] text-xs">•</span>
          <span className="text-[#8197a4] text-xs uppercase">{show.original_language}</span>
        </div>
      </div>
    </div>
  );
